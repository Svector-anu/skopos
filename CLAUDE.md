...
| `POST /api/sniper-check` | $0.05 | `scanToken()` — `lib/dexscreener.ts` — plus `getHolderConcentration()` (`lib/holderConcentration.ts`, Nansen `tgm/holders` via `fetchHoldersServer()` — same call the free "who holds $X" command uses, not a new integration). Priced for the concentration-only reality: `getSniperCheck()` (`lib/sniperCheck.ts`, HYRE Agent) is short-circuited to `null` — HYRE's Base endpoint confirmed 500ing on every well-formed payment, Solana has no signer, SKALE's challenge fails schema validation (see `docs/paid-data-sources.md`). Was $0.15 when sniper detection was believed to work; restore that price once it actually does. No free `/api/chat` equivalent exists yet for the bundle as a whole — this route has no free-path twin to reuse, unlike the rows above |
