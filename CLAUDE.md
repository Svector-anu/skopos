...
A2A_KPI_SEED             # optional — pre-counter A2A call history (~900), added at read time to the lifetime kpi:a2a:total counter by /api/stats; the Redis key itself only counts calls observed since the counter shipped
```
...
| `POST /api/sniper-check` | $0.25 | `scanToken()` — `lib/dexscreener.ts` — plus `getSniperCheck()` (`lib/sniperCheck.ts`, x402 Chain Intel, $0.18, Base-only — replaced HYRE Agent 2026-07-15, see `docs/paid-data-sources.md`) and `getHolderConcentration()` (`lib/holderConcentration.ts`, Nansen `tgm/holders` via `fetchHoldersServer()` — same call the free "who holds $X" command uses, not a new integration). RH Chain (SKALE) and Solana still blocked — no signer for Solana, SKALE's HYRE challenge fails schema validation. No free `/api/chat` equivalent exists yet for the bundle as a whole — this route has no free-path twin to reuse, unlike the rows above |
| `POST /api/flash-order` | $0.02 | `resolveFlashOrderLeg()` — `app/api/chat/route.ts` (exported). Advanced-order quotes (limit/stop-loss/take-profit/TWAP) on Robinhood Chain + 7 EVM chains; market swaps not included (Robinhood-only via the free chat path, Delora covers the rest through `/api/quote`) |
