...
| `POST /api/sniper-check` | $0.15 | `scanToken()` — `lib/dexscreener.ts` — plus `getSniperCheck()` (`lib/sniperCheck.ts`, HYRE Agent, $0.04, Base-only) and `getHolderConcentration()` (`lib/holderConcentration.ts`, Nansen `tgm/holders` via `fetchHoldersServer()` — same call the free "who holds $X" command uses, not a new integration). No free `/api/chat` equivalent exists yet for the bundle as a whole — this route has no free-path twin to reuse, unlike the rows above |
